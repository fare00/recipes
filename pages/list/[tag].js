...
export async function getServerSideProps({params, res}) {
    ...
    const query = params.tag == 'new' ? { content_type: 'recipe' } : { content_type: 'recipe', 'metadata.tags.sys.id[in]': params.tag };
...
function List({ recipes, tag, error }) {
    if (error) return <NotFound />

    return (
        <>
            <Head>
                <title>{tag.slice(0, 1).toUpperCase() + tag.slice(1, tag.length)} - Recipes</title>
            </Head>
            <main className="mt-32 max-w-7xl mx-auto px-5">
                <header className="flex items-center flex-col gap-5">
                    <span className="bg-green-500/20 text-green-800 uppercase text-lg p-2 font-bold">{tag}</span>
                    <h1 className="font-serif text-5xl">{tag.slice(0, 1).toUpperCase() + tag.slice(1, tag.length)} Recipes</h1>
                </header>
                <div className="flex flex-col gap-5 mt-10">
                    <h2 className="text-4xl border-b border-green-800 font-serif pb-5 text-green-800">Explore Meals</h2>
